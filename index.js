...
:root{
  --navy:#050814;--navy2:#0a0e1a;--navy3:#101828;
  --orange:#F26419;--amber:#FFAA44;
  --green:#4ade80;--red:#f87171;--blue:#60a5fa;--purple:#a78bfa;
  --border:#1a2035;--text:#e8ecf0;--muted:#4a5568;--mono:'DM Mono',monospace;
}
...
body::after{
  ...
  background:linear-gradient(
    180deg,
    rgba(5,8,20,0.68) 0%,
    rgba(6,10,24,0.52) 30%,
    rgba(8,12,28,0.58) 65%,
    rgba(5,8,20,0.90) 100%
  );
  z-index:-1;
}
.header{
  background:rgba(5,8,20,0.70);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);
...
.card{background:rgba(5,10,22,0.72);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);border-radius:14px;padding:.85rem;border:1px solid rgba(255,255,255,0.07)}
...
<div class="section" id="tab-budgets">
  <div class="grid"><div class="card full" id="b-ls">Chargement...</div></div>
</div>
...
<div class="section" id="tab-objectifs">
  <div class="grid"><div class="card full" id="o-ls">Chargement...</div></div>
</div>
...
function changerMois(dir){const n=moisOffset+dir;if(n>0)return;moisOffset=n;charger();}
...
const CHARGES_FIXES_LISTE=[
  {nom:'Loyer',montant:832.46},{nom:'Tontine 1',montant:500},{nom:'Tontine 2',montant:500},
  {nom:'Virement mère',montant:150},{nom:'Parking',montant:50},{nom:'Mutuelle',montant:57.03},
  {nom:'Énergie',montant:39.40},{nom:'Mobile',montant:17.99},{nom:'Box',montant:24},
  {nom:'Sport',montant:22.99},{nom:'Assurance hab.',montant:8.46},{nom:'Assurance auto',montant:64.24},
  {nom:'Sport (2)',montant:44},{nom:'Canal+',montant:13},{nom:'Cours arabe',montant:31},
  {nom:'Claude.ai',montant:21.60},{nom:'Helloasso',montant:12.55},{nom:'Stripe',montant:10},
  {nom:'Disney+',montant:6.99},{nom:'Crunchyroll',montant:8.99},{nom:'Banque',montant:18.30},
];
function renderChargesFixes(t){document.getElementById('a-cf-total').textContent=fmt2(t||2432.98);}
...
function renderRevenus(d){
  ...
  rco.textContent=fmt(d.completude);rco.style.color=col(cp);
...
async function charger(){
  try{
    ...
    sl.textContent=(d.solde>=0?'+':'')+fmt(d.solde);sl.style.color=cs(d.solde);
    ...
    co.textContent=fmt(d.completude);co.style.color=col(cp);
    ...
    document.getElementById('c-ls').innerHTML=d.cours.length===0?'<div style="color:var(--muted);font-size:.72rem;padding:6px 0">Aucun cours ce mois</div>':d.cours.map(c=>'<div class="cours-row"><span>'+c.eleve+(c.rattrapage?' <span style="color:var(--muted)">(rattrapage)</span>':'')+'</span><span class="green" style="font-family:var(--mono)">+'+c.gain.toFixed(2)+' €</span></div>').join('');
    if(d.nb_cours_manques>0){document.getElementById('c-mc').style.display='block';document.getElementById('c-ml').innerHTML=d.cours_manques.map(c=>'<div class="cours-row"><span>'+c.eleve+'</span><span class="red" style="font-family:var(--mono)">-'+c.gain_manque.toFixed(2)+' €</span></div>').join('');}
